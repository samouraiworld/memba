...
import { KNOWN_NETWORK_NAMES, resolveNetworkRpc } from "./networks.js";
...
server.registerTool(
  ...
  async ({ network }) => {
    const rpcUrl = resolveNetworkRpc(network);
    if (rpcUrl === null) {
      return {
        content: [{
          type: "text",
          text: `Unknown network "${network}". Use: ${KNOWN_NETWORK_NAMES.join(", ")}, or a full URL.`,
        }],
        isError: true,
      };
    }
