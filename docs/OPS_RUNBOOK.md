...
1. **Deploy first, read the sweep.** Every boot runs a read-only retro-sweep (`SweepMultisigSigVerify`) that re-verifies ALL stored signatures and publishes `memba_multisig_sig_verify_sweep{result}` (gauge) plus a `multisig_sig_verify_sweep: done` log line with `ok/mismatch/legacy_shape/error` totals. `legacy_shape` rows predate the canonical stored shape (pre-W2.4) and are expected to be non-zero on prod; they do NOT block the flip. **`mismatch` must be 0** — any mismatch row is logged individually with `tx_id`, `signer`, and `signed_at`; investigate before proceeding (a recent `signed_at` means the live reconstruction still diverges).
