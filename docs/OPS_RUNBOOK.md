...
| `ANALYST_DAILY_CAP_PER_WALLET` | Fly (backend) | AI analyst cost bound | Reports one wallet may generate per UTC day (default `5`). **`0` = wallets cannot generate reports** (cached reports are still served). Cache hits don't count. Counters live in `analyst_usage` and are purged daily. |
...
### 2.2 Block Party (arcade) — parked/enabled contract
...
→ immutable `blockparty_challenges` row. Public re-derivation:
`node scripts/verify-blockparty-seed.mjs` (defaults to the CANONICAL node —
...
deliberately carries no game flags).
