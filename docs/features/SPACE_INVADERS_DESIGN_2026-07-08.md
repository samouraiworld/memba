> **Historical design snapshot — v1, dated 2026-07-08.** Copied verbatim from the
> internal planning archive so the shipped game's design record lives with the code.
> Two things have moved since it was written:
>
> 1. **Shipped scope exceeded this spec** — destructible bunkers, the mystery UFO,
>    rapid fire (multi-bullet), and combo scoring all landed even though v1 scoped
>    them out.
> 2. **The "localStorage-only / no on-chain leaderboard" non-goal is SUPERSEDED**
>    by the owner-approved on-chain certification track (2026-09-01). The
>    deterministic-engine + fixed-timestep replay substrate this spec mandated is
>    exactly what certification builds on.
>
> Everything below is the original document, unchanged.
