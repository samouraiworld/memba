...
| Type/lint/theme/production security | `npx tsc --noEmit`, `npm run lint`, exact hardcoded text-color gate, and `npm run audit:ci`: **passed**; no unallowlisted high/critical production advisory. |
| Shared dependency advisory checks | **Externally blocked on the unchanged base dependencies.** The required Node 22 job passes coverage, bundle, and production audit, then fails `npm run audit:ci:dev`; the workflow's separate non-required `node scripts/pnpm-audit-ci.mjs` fails too. Both report the same four high `fast-uri` advisories against 4.1.2, patched in 4.1.3. Repair requires shared override plus npm/pnpm lockfile re-resolution. A separate `security-overrides` worktree is already active, so this isolated game branch does not race it or change protected lockfiles. |
...
Every Block Party functional, deterministic, backend, accessibility, and browser gate is green. The PR is nevertheless **not all-check-green or merge-ready** because the required Node 22 frontend job inherits the shared dev-dependency advisory failure recorded above. It remains a draft/blocked handoff until the dedicated dependency-security work lands or the branch is rebased onto that fix. This branch will not merge or deploy.
...
6. **Shared npm and pnpm dev trees have an unrelated high-advisory gate failure.** A dedicated dependency-security change must raise the `fast-uri` floor to at least 4.1.3 and re-resolve the affected lockfiles; that protected cross-workspace change is outside this game branch and already has a separate active worktree.
