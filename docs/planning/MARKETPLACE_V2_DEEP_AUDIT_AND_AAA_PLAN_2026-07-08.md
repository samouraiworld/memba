...
- **Task 1.5b — `<ReputationBadge>` on the NFT lane only.** Wire the existing `fetchReputation` (read-only, ungated read is acceptable for DISPLAY; the purchase-GATE for *writing* reviews is Phase 6) onto NFT `MarketCard`s + `TokenDetail`, keyed by seller address from the authoritative reviews realm. Show "New seller" when null. *Test:* card shows reputation or neutral state; never fabricated.
...
- **Task 2.0 — Funnel instrumentation (added per CTO #5).** Emit lightweight analytics events across the shipping path (view → detail → sign → settle) so the "P0s are felt" claim is falsifiable. *Acceptance:* events fire on the NFT lane; no PII; behind the existing analytics util.
- **Task 2.1** `codec.ts`: zod schemas + `parse<T>(): Result<T>` for each realm read (NFT v3 listings, OTC CSV, agent registry, reputation); never throws into render; failure → typed soft-error. *Test:* malformed field → `Result.err`, not a throw; valid → typed object; the OTC blind-`BigInt` and markdown-table paths get length/shape guards. 
- **Task 2.2** `useLaneQuery.ts`: `useInfiniteQuery` keyed `['market', lane, filters, cursor]`, cursor pagination, `select` maps rows → `UnifiedListing`. Replace `useEffect`+`setState` in all lanes. *Test:* tab switch doesn't refetch (cache hit); trade → `invalidateQueries` (no reload).
- **Task 2.3** Adapters `nftToCard`/`serviceToCard`/`tokenToCard`/`agentToCard`: `UnifiedListing → CardModel`. *Test:* each adapter maps a seed row to a valid `CardModel` with correct price label, stats, href.
- **Task 2.4** Virtualize `ListingGrid` with `@tanstack/react-virtual` behind the Phase-1 API. *Test:* 500-item book mounts a bounded window.
- *Acceptance:* every lane fetches through Query + codec; no raw `setState` book fetch; no `location.reload`; malformed realm data degrades softly (no `0 GNOT` masquerade).
