...
2026-07-16 · Lane B · Feed v2 Wave C.3 moderation-queue + audit-log RPCs · START · SINGLE-WRITER `api/memba/v1/memba.proto` (adds GetFlaggedPosts + GetModerationLog RPCs + their req/resp messages, NO change to Signature/verified — Lane C safe) + buf-regenerated `backend/gen/**` & `frontend/src/gen/**`; backend `internal/service/feed_moderation_rpc.go` (new) + test; consumes the feed_flags projection (#957, merged) + feed_raw_events (body-free). No CHANGELOG (backend RPCs, dark until C.4 console consumes them → no-changelog). ⚠️ MERGE-HOLD: backend merge auto-deploys Fly → owner-gated. branch feat/feed-moderation-rpcs
