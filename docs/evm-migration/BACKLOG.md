...
| **B-5** | The CAL is still **unmounted dead code** — `ChainContextProvider` is not in `main.tsx`, zero pages use `useChain()`, and 40 files still import `useAdena`. Plan §12 S052-058 ("87 pages migrated") has not started. | ✅ **Now unblocked by B-3** — mounting no longer breaks Gno broadcasts (the switchChain reload keeps `assertWalletBroadcastSafe` correct). Still a large migration (mount `ChainContextProvider`, wire one page end-to-end, then the ~40 `useAdena` sites). | One page wired end-to-end (the plan's Phase-V validation step), then the rest. |
...
| **G-8** | **`ci.yml`'s "Security audit" (`npm audit --audit-level=high`) fails REPO-WIDE, not from EVM-migration code.** A newly-published **high-severity react-router advisory** (GHSA-qwww-vcr4-c8h2, "RSC Mode CSRF Bypass", affects 7.12.0–8.2.0; installed `react-router-dom@^7.18.1`) + a dompurify advisory trip it. `ci.yml` went **success→failure between `4e21b214` and `ab4e9350` with NO dependency change** — the advisory DB updated, so it reds every push regardless of the diff. A frontend/contract-push `ci.yml` red is therefore expected and does **not** indicate a code defect. **Owner decision** (not remediation scope): the app is a Vite SPA that does not use react-router **RSC mode**, so the CSRF path is likely unreachable — options: (a) bump to a patched rr7 when released, (b) a scoped `npm audit` allowlist for the RSC advisory with rationale, or (c) `npm audit fix --force` (breaking downgrade to 7.11.0 — risky; the app relies on rr7 relative-splat behaviour). | ⬜ open — owner |
