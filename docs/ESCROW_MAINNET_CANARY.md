...
- The lane shows one curated listing, **Samourai Coop — dev services** ("Curated by Memba", "Quote per project"). Its Hire button opens the same form with the freelancer locked to `g1747t5m2f08plqjlrjk2q0qld7465hxz8gkx59c` and the title prefilled; if the connected wallet is that address, the form refuses up front (no self-hire). The canary uses the free-form **Hire by address**, because F is one of the owner's personal wallets.
