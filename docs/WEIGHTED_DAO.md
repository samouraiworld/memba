...
**Wallet and chain checks.** Right before signing a v12 call, the page asks the wallet for its network again (account and network), and refuses when the chain id is empty or unknown, differs from the page's chain, is on the hold list (gnoland-1), the RPC is untrusted, or the account changed. Target authority, proposal and proposal-page re-reads each go through the RPC chain check. The one-open-acceptance rule pages through the whole proposal history (up to 25 pages of 20) and refuses rather than answer from a partial read.

**Before the mainnet hold is lifted.**
- Re-measure the executions that have no measurement yet, at least `recover-member` and `set-role:remove` (they currently use the largest measured execution, 81.9M gas and a 1.46 GNOT cap), and re-check `Vote` gas growth against the live proposal count: it rose from 15.2M at proposal 1 to 16.2M at proposal 71, and the 24.8M limit covers several hundred proposals, not an unbounded history.
- Follow-up outside this change: the shared broadcast guard (`assertWalletBroadcastSafe` in `grc20.ts`) skips its wrong-chain check when the cached wallet chain id is empty. It applies to every signing path, so it is fixed separately; v12 writes do not rely on it.
