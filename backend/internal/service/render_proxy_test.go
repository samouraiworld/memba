...
func TestGnoRPCURL_DefaultsToMainnet(t *testing.T) {
	t.Setenv("GNO_RPC_URL", "") // force the built-in default
	const want = "https://rpc.gno.land:443"
	if got := gnoRPCURL(); got != want {
		t.Fatalf("gnoRPCURL() default = %q, want the gno.land mainnet canonical node %q (retired-chain drift?)", got, want)
	}
}
