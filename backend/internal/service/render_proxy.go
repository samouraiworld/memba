...
// the public gno.land mainnet canonical (gnoland-1) — the chain this release
// serves since the 2026-09-23 cutover — so an environment that forgets to set
// GNO_RPC_URL reads the right chain rather than a retired one. The public node
// rate-limited the Fly egress IP under sustained polling on topaz (#466,
// presenting as 403): low-volume proxy reads are fine, but keep the feed tailer
// on a DIFFERENT node (the Samourai mainnet node, rpc.mainnet.samourai.live)
// and set GNO_RPC_URL to a dedicated node if throttling reappears.
...
// and defaults to the public mainnet node, keeping marketplace reads decoupled
