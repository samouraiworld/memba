                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 10px", justifyContent: "center" }}>
                    {segments.map(seg => (
                        <div key={seg.key} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                            ...
                            <span style={{ fontSize: "var(--pro-caption, 9px)", fontFamily: "var(--font-ui, JetBrains Mono, monospace)", color: "var(--color-text-secondary)" }}>{seg.key}</span>
                        </div>
                    ))}
                </div>
            )}
                ...
                {tiers.map((t, i) => {
                    ...
                    return (
                        <div key={t.tier} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            <div>
                                <div style={{ fontSize: "var(--pro-small, 12px)", fontWeight: 600, color: "var(--color-text)", fontFamily: "var(--font-ui, JetBrains Mono, monospace)" }}>
                                    {t.tier}
                                </div>
...
export function VotingInsights({ yesVotes, noVotes, abstainVotes, totalMembers, threshold, voteRecords }: {
    ...
    const labelStyle: React.CSSProperties = {
        fontSize: "var(--pro-caption, 10px)", fontFamily: "var(--font-ui, JetBrains Mono, monospace)",
    ...
    return (
        <div className="k-card" style={{ padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
            <h3 style={{ fontSize: "var(--pro-body, 14px)", fontWeight: 600, color: "var(--color-text)", margin: 0, display: "flex", alignItems: "center", gap: 8 }}>
                📊 Voting Insights
            </h3>
            ...
            <div style={sectionStyle}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
                    <span style={{ fontSize: "var(--pro-small, 12px)", fontFamily: "var(--font-ui, JetBrains Mono, monospace)", color: "var(--color-text)", fontWeight: 600 }}>
                        {totalVoted} of {totalMembers} voted
                    </span>
                    <span style={{ fontSize: "var(--pro-small, 12px)", fontFamily: "var(--font-ui, JetBrains Mono, monospace)", color: quorumMet ? "var(--color-brand)" : "var(--color-accent-gold)", fontWeight: 700 }}>
                ...
                {/* Quorum status */}
                <div style={{ marginTop: 6, fontSize: "var(--pro-caption, 10px)", fontFamily: "var(--font-ui, JetBrains Mono, monospace)" }}>
                    {quorumMet ? (
            ...
            <div style={sectionStyle}>
                ...
                {/* Vote counts row */}
                <div style={{ display: "flex", gap: 16, marginBottom: 6, fontSize: "var(--pro-small, 12px)", fontFamily: "var(--font-ui, JetBrains Mono, monospace)" }}>
                            ...
                            {voteRecords.filter(r => r.yesVoters.length + r.noVoters.length + r.abstainVoters.length > 0).map((r, i) => (
                                <div key={r.tier} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ fontSize: "var(--pro-caption, 10px)", fontFamily: "var(--font-ui, JetBrains Mono, monospace)", display: "flex", gap: 8 }}>
