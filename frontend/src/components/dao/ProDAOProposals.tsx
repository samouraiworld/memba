...
const matches = (p: DAOProposal, filter: Filter) => filter === "All" ||
    (filter === "Open for voting" ? p.status === "open" : filter === "Awaiting execution" ? p.status === "passed" : p.status === "executed" || p.status === "rejected")
const statusLabel = { open: "Open for voting", passed: "Awaiting execution", executed: "Executed", rejected: "Rejected" }
