...
// CI's GNO_PIN; either way the refund proof below would be meaningless.
...
it("gno toolchain is coherent when the gate is required (REQUIRE_GNO=1)", () => {
    if (REQUIRE_GNO) {
        expect(
            TOOLCHAIN.ok,
            `REQUIRE_GNO=1 but the gno toolchain cannot run the refund proof — ${TOOLCHAIN.message}\n${TOOLCHAIN.lines.join("\n")}`,
        ).toBe(true)
    }
})
