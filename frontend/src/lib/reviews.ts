...
// reads + write builders default to REVIEWS_PKG_PATH but accept an explicit `realm` so a caller
...
async function evalJSON(expr: string, realm: string = REVIEWS_PKG_PATH): Promise<string> {
    const raw = await queryEval(GNO_RPC_URL, realm, expr)
 ...
 */
export async function fetchReviews(subject: string, offset = 0, limit = 20, realm: string = REVIEWS_PKG_PATH): Promise<OnChainReview[]> {
    const json = await evalJSON(`GetReviewsJSON(${JSON.stringify(subject)}, ${offset}, ${limit})`, realm)
 ...
 */
export async function fetchComments(reviewID: number, offset = 0, limit = 50, realm: string = REVIEWS_PKG_PATH): Promise<OnChainComment[]> {
    const json = await evalJSON(`GetCommentsJSON(${reviewID}, ${offset}, ${limit})`, realm)
    return parseComments(json)
}

/** Fetch aggregate summary for a subject (count / average / sum). */
export async function fetchSummary(subject: string, realm: string = REVIEWS_PKG_PATH): Promise<SubjectSummary> {
    const json = await evalJSON(`GetSubjectSummaryJSON(${JSON.stringify(subject)})`, realm)
    try {
        const v = JSON.parse(json)
 ...
 */
export async function fetchReputation(addr: string, realm: string = REVIEWS_PKG_PATH): Promise<number> {
    const raw = await queryEval(GNO_RPC_URL, realm, `GetReputation(${JSON.stringify(addr)})`)
 ...
 */
function buildReviewMsgCall(func: string, args: string[], caller: string, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return { type: "vm/MsgCall", value: { caller, send: "", pkg_path: realm, func, args } }
}
 ...
 */
export function buildPostReviewMsg(caller: string, subject: string, rating: number, body: string, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("PostReview", [subject, String(rating), body], caller, realm)
}
 ...
 */
export function buildEditReviewMsg(caller: string, reviewID: number, rating: number, body: string, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("EditReview", [String(reviewID), String(rating), body], caller, realm)
}
 ...
 */
export function buildDeleteReviewMsg(caller: string, reviewID: number, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("DeleteReview", [String(reviewID)], caller, realm)
}
 ...
 */
export function buildReactMsg(caller: string, targetID: number, kind: "like" | "dislike", realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("React", [String(targetID), kind], caller, realm)
}
 ...
 */
export function buildCommentMsg(caller: string, reviewID: number, body: string, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("PostComment", [String(reviewID), body], caller, realm)
}
 ...
 */
export function buildEditCommentMsg(caller: string, commentID: number, body: string, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("EditComment", [String(commentID), body], caller, realm)
}
 ...
 */
export function buildDeleteCommentMsg(caller: string, commentID: number, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("DeleteComment", [String(commentID)], caller, realm)
}
 ...
 */
export function buildFlagMsg(caller: string, targetID: number, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("Flag", [String(targetID)], caller, realm)
}
 ...
 */
export function buildHideReviewMsg(caller: string, reviewID: number, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("HideReview", [String(reviewID)], caller, realm)
}
 ...
 */
export function buildHideCommentMsg(caller: string, commentID: number, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("HideComment", [String(commentID)], caller, realm)
}
 ...
 */
export function buildUnhideMsg(caller: string, targetID: number, realm: string = REVIEWS_PKG_PATH): AminoMsg {
    return buildReviewMsgCall("Unhide", [String(targetID)], caller, realm)
}
