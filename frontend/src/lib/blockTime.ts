...
/**
 * Format a relative time string: "just now", "3 minutes ago", "2 hours ago", "5 days ago".
 *
 * @param date - The date to compare against now
 * @param nowMs - Current time in ms (defaults to Date.now(), injectable for tests)
 * @returns Human-readable relative time string, or "" if date is invalid
 */
export function formatRelativeTime(date: Date | null, nowMs: number = Date.now()): string {
    if (!date || isNaN(date.getTime())) return ""

    const diffMs = nowMs - date.getTime()
    if (diffMs < 0) return "just now" // future dates (shouldn't happen, but safe)
