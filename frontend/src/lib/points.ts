...
// Guard the qeval expression against injection: only well-formed gno addresses are ever interpolated.
...
export async function getProfile(address: string): Promise<PointsProfile | null> {
    if (!ADDR_RE.test(address)) return null
    const raw = await queryEval(GNO_RPC_URL, REALM_PATH, `ProfileJSON(${JSON.stringify(address)})`)
    if (!raw) return null
...
export async function getTopN(n: number): Promise<LeaderRow[]> {
    const count = Math.max(1, Math.min(200, n | 0))
    const raw = await queryEval(GNO_RPC_URL, REALM_PATH, `TopN(${count})`)
    if (!raw) return []
    const v = parseQevalJSON(raw)
...
export async function getTopNPage(offset: number, count: number): Promise<LeaderRow[]> {
    const off = Math.max(0, offset | 0)
    const cnt = Math.max(1, Math.min(200, count | 0))
    const raw = await queryEval(GNO_RPC_URL, REALM_PATH, `TopNPage(${off}, ${cnt})`)
    if (!raw) return []
    const v = parseQevalJSON(raw)
    return Array.isArray(v) ? v.filter(isLeaderRow) : []
}
...
export async function getTierBands(): Promise<TierBand[]> {
    const raw = await queryEval(GNO_RPC_URL, REALM_PATH, "TierBandsJSON()")
    if (!raw) return []
    const v = parseQevalJSON(raw)
