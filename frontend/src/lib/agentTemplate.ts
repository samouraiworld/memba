...
import { escapeGnoString, sanitizeString, validateRealmPath as _validateRealmPath } from "./templates/sanitizer"
...
export function generateAgentRegistryCode(config: AgentRegistryConfig): string {
    ...
    // W1.1 R2-GEN-A: the header comment interpolates the raw name — a newline
    // in it broke out of the comment. Comments need control chars stripped,
    // not literal-escaping, so sanitizeString (not safeName) is used here.
    const commentName = sanitizeString(config.name, 80)
    return `package ${pkgName}

// ${commentName} — On-chain AI Agent Registry
