...
import { getProposalDetail, parseProposalList } from './proposals'
...
const detail = () => getProposalDetail('https://rpc.example', 'gno.land/r/gov/dao', 4)
...
describe('proposal detail status fields', () => {
    ...
    it.each([
        ['PROPOSAL HAS BEEN ACCEPTED', 'passed'],
    ...
    it('supports CRLF-rendered status metadata', async () => {
        ...
        expect((await detail())?.status).toBe('passed')
    })
})
