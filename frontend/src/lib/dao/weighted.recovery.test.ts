...
it("checks the recovery checksum, exact current seat and unused replacement", async () => {
    ...
    expect(buildWeightedMessage(fixture.members[1].address, weightedRealm, action, fixture.config.schema, "pearl").value).toMatchObject({ func: "ProposeRecovery", args: [action.personId, action.oldAddress, replacement], send: "" })
