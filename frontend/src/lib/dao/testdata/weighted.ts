...
export function weightedFixture(version: 1 | 2 = 1) {
    ...
    const config = { schema: `memba-weighted-host/v${version}`, kind: "config", realmPath: weightedRealm, rosterSize: 7, totalPoints: 8, founderWeight: 2, developerWeight: 1, votingPeriodSeconds: 604800, maxProposalPage: 50,
        ...
        capabilities: { roleProposals: true, memberReplacement: version === 2, migration: false, treasuryExecution: false, applicationActions: false },
    }
    const proposal: WeightedProposal = { id: "1", proposer: members[0].address, action: { type: "set-role", target: members[1].address, role: "admin", grant: true }, category: "critical", status: "VOTING", qualified: false, ready: false, votingClosed: false, talliesAvailable: true, weightYes: 2, peopleYes: 1, developersYes: 0,
