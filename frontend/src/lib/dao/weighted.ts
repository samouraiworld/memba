...
const envelope = { schema: z.literal(WEIGHTED_SCHEMA) }
export const weightedConfigSchema = z.strictObject({
    ...
    capabilities: z.strictObject({ roleProposals: z.literal(true), memberReplacement: z.literal(false), migration: z.literal(false), treasuryExecution: z.literal(false), applicationActions: z.literal(false) }),
})
const member = z.strictObject({ personId: z.string().min(1).max(320), address, founder: z.boolean(), weight: z.union([z.literal(1), z.literal(2)]), admin: z.boolean(), finance: z.boolean() })
...
const proposal = z.strictObject({
    id, proposer: address, action: z.strictObject({ type: z.literal("set-role"), target: address, role, grant: z.boolean() }),
...
export const weightedProposalSchema = z.strictObject({ ...envelope, kind: z.literal("proposal"), proposal })
export const weightedPageSchema = z.strictObject({ ...envelope, kind: z.literal("proposals"), total: uint64, proposals: z.array(proposal).max(50), nextBefore: id.nullable() })
...
export type WeightedAction = { type: "propose"; target: string; role: "admin" | "finance"; grant: boolean } | { type: "vote"; id: string; vote: "yes" | "no" | "abstain" } | { type: "execute"; id: string }
...
export async function readWeightedSnapshot(ctx: WeightedContext, before = "0", signal?: AbortSignal) {
    ...
    for (const p of page.proposals) {
        if (BigInt(p.id) !== previous - 1n || BigInt(p.id) > BigInt(page.total) || !roster.members.some(m => m.address === p.proposer) || !roster.members.some(m => m.address === p.action.target)) throw new Error("Invalid proposal page")
...
export async function readWeightedProposal(ctx: WeightedContext, proposalId: string) {
    id.parse(proposalId)
    const result = weightedProposalSchema.parse(await read(ctx, `GetProposalJSON(${proposalId})`)).proposal
...
export function buildWeightedMessage(caller: string, realmPath: string, action: WeightedAction): AminoMsg {
    ...
    if (action.type === "propose") { func = "ProposeRole"; args = [address.parse(action.target), role.parse(action.role), String(z.boolean().parse(action.grant))] }
