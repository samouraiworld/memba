...
describe("weighted contract", () => {
    ...
    it("preserves uint64 IDs and exact governed entrypoints without sent coins", () => {
        ...
        expect(buildWeightedMessage(caller, weightedRealm, { type: "vote", id: max, vote: "yes" }).value).toEqual({ caller, send: "", pkg_path: weightedRealm, func: "Vote", args: [max, "yes"] })
        expect(buildWeightedMessage(caller, weightedRealm, { type: "execute", id: max }).value.func).toBe("Execute")
        expect(buildWeightedMessage(caller, weightedRealm, { type: "propose", target: fixture.members[1].address, role: "finance", grant: false }).value.args).toEqual([fixture.members[1].address, "finance", "false"])
        for (const id of ["0", "01", "-1", "1e3", "18446744073709551616", '1);panic("x")']) expect(() => buildWeightedMessage(caller, weightedRealm, { type: "execute", id })).toThrow()
    })
    it("blocks mainnet and stale wallet/network contexts unconditionally", () => {
        expect(() => assertWeightedWrites("gnoland-1", "gnoland-1", "gnoland-1")).toThrow("hold")
        expect(() => assertWeightedWrites("pearl", "pearl", "other")).toThrow()
        expect(() => assertWeightedWrites("pearl", "other", "pearl")).toThrow()
        expect(() => assertWeightedWrites("pearl", "pearl", "pearl")).not.toThrow()
    })
})
