...
export async function accountReviewFixture(page: Page) {
    await page.addInitScript(({ address }) => {
        ...
        localStorage.setItem('memba_auth_token', JSON.stringify({ nonce: 'design-fixture', userAddress: address, expiration: '2099-01-01T00:00:00Z', chainId: 'pearl', serverSignature: 'invalid-test-only' }))
        ...
        const api = {
            GetAccount: async () => ({ status: 'success', data: { address, coins: '0ugnot', publicKey: { '@type': '/tm.PubKeySecp256k1', value: 'A6+DHJsdkWFczHKaLWvmPIIQhjIQRYHrSzqFZGsrwJfE' }, accountNumber: '0', sequence: '0', chainId: 'pearl' } }),
    ...
    await page.route('**/memba.v1.MultisigService/*', route => {
        ...
        const readResponses: Record<string, unknown> = {
            ...
            GetTransaction: { transaction: {
                ...
                chainId: 'pearl', threshold: 2, membersCount: 3, memo: 'Community operations — design fixture',
