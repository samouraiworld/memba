...
only after a **fee-path checklist** was verified on-chain: the treasury address
matches, the coin-guard rejects overpay and wrong denominations, `IsUserCall`
rejects realm-to-realm calls, and every `RegisterApp` variant forwards its fee.
Accounting was checked end to end — start balance minus end balance equals N ×
the forwarded fee.
