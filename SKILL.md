...
**Default RPC:** `https://rpc.gno.land:443` (gno.land mainnet, chain id `gnoland-1`; fallback `https://rpc.mainnet.samourai.live:443`)
...
```bash
DATA=$(printf 'gno.land/r/gov/dao:' | base64)
...
curl -s https://memba-backend.fly.dev/memba.v1.MultisigService/Multisigs \
  ...
  -d '{"userAddress":"g1ADDRESS","chainId":"gnoland-1"}'
```
