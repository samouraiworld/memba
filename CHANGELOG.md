...
- **"My submissions" gains Delist** (any non-delisted listing, free): an armed two-step confirm spells out the honest contract before signing — delisting is one-way for the publisher, only a curator can restore it, and the package path stays taken. Live rows also explain why they can't be edited (verified content is locked; a curator updates it).
