...
- **Fixed the Directory reporting realms as undeployed, and its drawers linking to the wrong network.** The Directory checks which of our realms are actually live by asking gnoweb for the namespace listing, using a second, separately maintained list of addresses that was never given a Topaz entry. With no address to ask, the check was skipped in silence — nothing was ever marked live — and the drawers fell back to mainnet `gno.land`, where our realms do not exist. Both now read the same per-network address, so adding a network cannot leave one of them behind again.
- Also removed the last places the app still treated the retired `test13` testnet as the live one: the Tokens and NFT marketplace lanes fell back to it when no network was in the URL, and the Team Hub's "Data: mainnet" note — which tells you the roster comes from gnolove's mainnet source rather than the chain you are on — was only ever shown on `test13`, so the cutover silently removed it. Old `test13` links still resolve.
