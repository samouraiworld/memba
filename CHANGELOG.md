...
### Weighted DAO v12: adapter acceptance, ballots and execution on test networks (2026-09-24)
<!-- categories: memba, network -->
- **Hand an application over to the DAO.** Each adapter card reads its target's current and pending authority and says whether it is awaiting the publisher's nomination, ready for the DAO to accept, nominated but refused by a host precondition, or already controlled by the DAO. When ready, a member can propose the acceptance (`Propose<Adapter>Accept`); it is re-checked before signing, and only one acceptance can be open at a time because executing any application action invalidates every other open proposal. After an acceptance executes, the page reads the target again and reports the handoff.
- **Hand the targets over in the recommended order.** The adapter cards are numbered in the handoff order (market config first, escrow last), the next target the DAO does not control yet is marked, each card says what changes once the DAO accepts it, and the page shows the path of an acceptance: propose, 6 points from at least 4 people and 24 hours (or 5 developers and 72 hours), then execute.
- **Vote and execute with your ballot in view.** Vote buttons follow your own ballot: nothing until it is read or if you are not eligible, never the choice you already cast, and a change allowed until voting closes. Execute first lists the open proposals it will invalidate.
- **Measured costs.** Every call carries a gas limit and a storage-deposit cap sized from measurements of the generated realm (1.5 × the highest measured value): about 2.1–2.3 GNOT per acceptance (4.34 GNOT for escrow), 0.04 GNOT per ballot. The confirmation shows the exact cap, and caps above 10 GNOT are refused.
- **Mainnet stays on hold.** On gnoland-1 no transaction is built for this contract and every control stays disabled, even for a connected member. On top of the shared wallet-network check before signing, a wallet on gnoland-1 is refused for these calls. Role and key-recovery proposals for this contract version come later.
...
- **One entry point.** Opening a weighted DAO from the DAO pages goes to its workspace.
...
- **Every escrow contract has its own page**, `/:network/marketplace/services/contract/<id>`, reachable from the Services lane and shareable. It shows what the realm reports (both parties in full, milestones with status, amounts and deadlines, totals, the fee terms) and only the calls the connected wallet can make in that state, as the realm's guards allow them: the client funds (sending exactly the milestone amount), releases, disputes, cancels and archives; the freelancer marks a milestone delivered or disputes it; anyone refunds a stale milestone, settles an expired dispute or expires a never-funded contract once the realm's pause-adjusted deadline has passed. While escrow is paused, funding is refused and every other call waits until exits reopen. Each call goes through the confirmation dialog and is never re-sent; when its outcome is unknown the page offers nothing until the contract is read again.
- **Hire by address.** A client can hire someone they already know: freelancer address (checksum-checked), title, description and milestones in GNOT, checked exactly as the realm checks them, with what the contract commits spelled out (per-milestone escrow, the storage deposit refunded on archive, the fee at release, at most 5 open contracts). Once created, Memba opens the new contract's page with a link to share with the freelancer.
- **One curated listing: Samourai Coop — dev services.** The Services lane shows it, marked "Curated by Memba", with the freelancer address in full and "Quote per project" instead of a price. Its Hire button opens the Hire by address form with that address locked and the title prefilled; the client writes the milestones and amounts agreed with the team. Curated listings live in `lib/marketplace/curatedServices.ts` and change only through reviewed pull requests.
- **Freelancers can find their contracts.** escrow_v4 lists contracts by client only, so the shared link is the dependable path. On mainnet the Services lane also lists "Contracts where you are the freelancer", found through the tx-indexer's `ContractCreated` events and each read back from the realm before it is shown.
...
- **Storage deposits come back.** Clients can archive a completed or cancelled contract ("Archive and reclaim deposit (~X GNOT)"); the chain refunds the freed deposit to the signer. Anyone can expire a never-funded contract from the realm's pause-adjusted expiry height. The hire dialog now says the deposit is refunded on archive, instead of "not refunded".
- **My contracts.** The Services lane lists the connected client's contracts, newest first and paged, and opens any of them. After a hire lands, it reads the new contract back and names its id.
...
- **Structured reads.** Contracts, the client's contract list and the pause state come from the realm's JSON views (`GetContractJSON`, `GetClientContractsJSON`, `GetPauseStateJSON`), parsed strictly: any unexpected shape is refused instead of shown. The node's string quoting is decoded in full, so text with newer emoji or private-use characters (printed as `\U` escapes) still reads.
...
- **Attesting is sized for mainnet.** Each `RecordCompletion` now sends a 1.6 GNOT storage-deposit cap and a 50M gas limit, measured against the deployed realm (up to 6.7 KB and 25M gas at 16,000 entries per tree). Before, it sent no cap (mainnet allows up to 100 GNOT) and the 10M default gas, which ran out after a few dozen records (9.3M gas at five). The panel warns that up to 1.6 GNOT per quest stays locked with the record (about 0.3–0.7 GNOT in practice). A lost wallet response is no longer retried, since the retry could only fail on the used nonce.
...
- **The escrow builders match the deployed realm and refuse what it would reject.** Function names and argument order are pinned to the published source in tests. Addresses, title and description sizes (in bytes), milestone count, titles and amounts are checked before signing, and amounts must be whole ugnot. Text containing a character the contract strips before storing it (square brackets, parentheses, `#`, `*`, backtick, `!`, `<`, `>`, `|`, backslash, `_`, `~`, tabs, line breaks) is refused with the character named, so the stored text is always the signed text. The old uncapped escrow call builders in `escrowTemplate.ts` are removed. Only funding a milestone sends coins, and it sends exactly that milestone's amount.
- **Hiring shows what it signs.** The hire dialog previews the milestones, the total to fund later and the deposit cap from the same plan the wallet signs, and says that nothing is sent when the contract is created. It also states that the storage deposit, up to this contract's exact cap, is not refunded, because contracts stay on-chain permanently. If signing fails in a way that may still have created the contract (a timeout, a network error, anything unrecognised), the dialog does not offer a plain retry: it links to the escrow contracts to check first and only offers "Create anyway" after the user confirms that no contract was created. Escrow stays gated: nothing is broadcast unless the services lane is enabled and `escrow_v3` is listed for the network, which it is not on mainnet yet.
### The arcade attester pays mainnet gas prices, not a flat 1 GNOT (2026-09-24)
<!-- categories: memba, network -->
- **The attestation fee follows the chain's gas price.** At boot, the day-close attester reads `auth/gasprice` from a node it has confirmed serves `GNO_CHAIN_ID`. It then pays twice the minimum fee for its gas budget: 0.1 GNOT per attestation on gno.land mainnet today, down from 1 GNOT. If the read fails, it uses the mainnet launch price.
- **The gas budget fits the mainnet realm.** It is now 50M, up from 5M. The old budget ran out of gas on the realm's very first entry. Operators can set `MEMBA_ARCADE_GAS_WANTED` and `MEMBA_ARCADE_GAS_FEE_UGNOT`. A fee above `MEMBA_ARCADE_MAX_GAS_FEE_UGNOT` (default 0.2 GNOT) keeps the attester off, with an error in the log. So does a malformed value (for example `50_000` or `100000ugnot`), or an RPC that serves a different chain.
...
- Allow the App Store, reviews, feedback, badges and feed realms on mainnet. Escrow, market config, DAO channels, quest attestation and the arcade leaderboard stay gated there until their own setup is complete.
...
- Carry the approved defender and machine plates into the compact 2D phone fallback, replacing its placeholder block wall with cobble and recovered cart timber.
...
<!-- categories: memba, network -->
- **Create DAO is available on the gno.land network.** Memba deploys a version-2 DAO under your own address or registered name, waits for gno.land to enable the package, and then opens it. The review step shows the network, the package path and the storage deposit cap before you sign. The channels companion is not offered on gno.land.
...
### Block Party, dark and kill-switched (#1128, #1130, #1131, #1143, 2026-09-04)
<!-- categories: memba -->
- **Production Daily is still deliberately paused by the backend kill switch** (`BLOCKPARTY_ENABLED`), which now disables the whole feature rather than one endpoint. The daily seed defaults to Pearl and is read from a single node whose chain identity is verified before any seed is trusted. Nothing in this release enables, deploys, reseeds, or alters any live chain or stored score.
- **The board you play is the board the server issued.** A cold load used to start you on a placeholder board before the day's challenge arrived (and never re-seeded), so ranked replays were scored against a board you never saw; the daily now re-seeds the moment it loads, and a background refresh can no longer wipe a run in progress. The result sheet appears only after a real seeded run ends — no more phantom "Round complete" over a dead board — and a failed fetch shows a plain error notice with Retry (plus the Practice escape hatch).
...
- **The headline claim is finally visible**: each daily shows *which* Pearl block seeded it — height, hash and a Verify link to the public re-derivation script — instead of fetching that proof and never rendering it. The game also has a new Pearl-native signal-board presentation: Daily and Practice are clearer, guest bests and streaks persist, share links retain their UTC date without private data, and the board supports accessible keyboard and captured touch input, reduced motion, light/dark themes, screen-reader context, and 320 px through short-landscape layouts.
...
### Space Invaders v1 (#1132, #1134, #1135, #1145, 2026-09-05)
<!-- categories: memba -->
- **The route is on, certification is off.** The game ships behind `VITE_ENABLE_SPACE_INVADERS`; the daily certify lane (day-close attestation and the on-chain leaderboard) stays behind `VITE_ENABLE_SPACE_INVADERS_CERTIFY`, which remains off until its owner ceremony.
...
- **Every tablist shares one keyboard contract (#1101, #1102, #1103).** Arrow-key navigation and focus handling were hand-rolled fifteen times; they are now one shared implementation, adopted across eight more tablists and the gnolove surfaces.
- **react-query waves 1–6 (#1104, #1105, #1106, #1108, #1110, #1111).** Three money-path pages, DAOMembers, MultisigHub, CandidaturePage, ChannelsPage, ProfilePage, eight more pages, DAOList, DAOHome and the fetch-shaped component tier moved off fetch-in-effect onto react-query, so their data is cached, deduplicated and refreshed consistently.
- **The set-state-in-effect ratchet is locked at zero (#1097, #1112)**: every remaining warning resolved, and the lint ratchet can only go down.
- **Stale-network sweep (#1109)**: the last live references to dead chains retired.
...
- **The arcade multi-game certify realm** is not deployed on Pearl. The certify pipeline fans out to multiple games but ships dark by default (#1133): with `MEMBA_ARCADE_GAMES` unset only BARRICADE is accepted, exactly as before.
- **Commerce and the marketplace** — tokens, collectibles, services (escrow) and OTC — are deployed on Pearl but switched off until the two-wallet live-money check passes; the **NFT indexer** stays disabled until observability is wired, and marketplace registration waits on it.
- **Marketplace v2** stays behind its flag, and **reactions** stay off.
- Mainnet is not authorized; nothing in this release targets it.
...
- **The social feed starts fresh.** Feed posts live on the chain that hosted them, and that chain is gone — showing the old posts as if they were current would have been a lie (and, technically, would have silently swallowed new posts, because post numbering restarts on the new chain). The Topaz feed history is preserved in our backups; the live feed begins again on Sapphire.
- **Temporarily unavailable, on purpose: creating tokens, and everything that moves funds.** The token factory, NFT marketplace, escrow and OTC trading were deliberately NOT redeployed in this first wave. Redeploying a contract bakes its settings in permanently, and one of them (the token factory's fee) deserves a decision rather than a copy-paste. Those lanes stay switched off — with their build-time safety gate re-armed — until their own deployment ceremony.
- **Old Topaz links stay honest.** A bookmark to a `/topaz/…` page still resolves; you get the degraded "chain unreachable" view and a working network switcher instead of a blank or fake-live app — the same treatment retired networks have had since test13.
- **For agents and integrators:** the published endpoints (`llms.txt`, the SKILL guide, README) now point at Sapphire — the login example's chain id included, since the old one is now actively refused.
<!-- categories: memba -->
...
- **A stale Betanet selection now heals itself — including your bookmarks.** If your browser remembered Betanet, it is restored as Topaz instead of leaving you on a network you can no longer pick from the switcher. This applies to saved links too: an older bookmark without a network in its address used to reopen on Betanet even though the home page had already moved you off it. Betanet links you follow directly still open, and the switcher always lists the network you are actually on, so you can always move off it.
- **The webhook chain field no longer goes blank on an alert saved for a hidden chain.** Editing a webhook scoped to Betanet showed an empty chain selector while quietly keeping the old chain on save, so you could neither see what it was set to nor change it. The saved chain is now always listed, marked as no longer offered.
- **The "chain unavailable" notice no longer suggests switching to a network without our contracts.** It offered Betanet whenever the main chain was struggling; it now stays silent rather than sending you somewhere nothing works.
...
- **Fixed the Directory's realm and token drawers linking to the wrong chain.** Those drawers built their "view on gnoweb" address from a second, separately maintained list that was never given a Topaz entry, so they fell back to mainnet `gno.land` — where our realms do not exist and the link 404s. Betanet was pointed at mainnet too, and now uses its own explorer. Both the drawers and the app's other explorer links now read one address per network, so adding a network cannot leave one of them behind again.
- Also removed the last places the app still treated the retired `test13` testnet as the live one: the Tokens and NFT marketplace lanes fell back to it when no network was in the URL, and the Team Hub's "Data: mainnet" note — which tells you the roster comes from gnolove's mainnet source rather than the chain you are on — was only ever shown on `test13`, so the cutover silently removed it — it now appears on any test chain, and stays hidden on Betanet. Old `test13` links still resolve.

### Feed — closed part of the moderation blocklist bypass on the public render API (#1031, 2026-07-30)
<!-- categories: memba -->
- **A blocklisted feed post could still be read in full through `/api/render`'s direct per-post path, bypassing the operator takedown.** `MODERATION_POLICY.md` already documented that this endpoint relays the chain directly and isn't filtered by our serving-side blocklist — honest, but incomplete: a single post is addressable by id (`?path=post/<id>`), so that one path is now checked against the blocklist before the chain is ever queried. A blocklisted post reads there exactly as a chain-hidden one does. The check also covers **replies**: the realm prints a post's replies in full underneath it, so a blocklisted reply was readable through its parent even when its own permalink was suppressed — requesting that parent is now suppressed too. Page and per-user listings through the same endpoint, and `/api/indexer`, are unchanged — still a direct chain relay, as documented, and blocklisting still needs to be paired with an on-chain removal for content that must not be served anywhere.
...
<!-- categories: memba -->
- **Fixed a timing gap where disconnecting your wallet could be silently undone.** Connecting to Adena involves a short exchange with the extension, and Memba also re-connects silently in the background when you return to the app. If you hit Disconnect — in that tab or another — during one of those in-flight exchanges, the finishing connection could overwrite your decision: the app showed you as connected again without asking. A connection that finds your disconnect mid-flight now stands down instead of completing, and the background reconnect can no longer re-create the session your disconnect just cleared.

### Directory — a network outage no longer empties the DAOs tab (#1027, 2026-07-30)
<!-- categories: memba -->
- **Fixed the Directory's DAO list silently hiding every DAO — including your saved ones — during a temporary network outage.** Like the home board before it, the DAOs tab could not tell "this DAO is not deployed on the current network" apart from "no RPC server answered just now", and treated both the same way: the entry vanished, leaving a misleading "No DAOs found". The two cases are now distinguished. When the chain itself answers that a realm is not deployed here, the stale entry is still filtered out as before; when nothing answers at all, your DAOs stay on the board as cards marked "couldn't reach chain" and recover on their own once the network is reachable again.
...
- **The social feed now reads and writes on Topaz.** The activity feed also works on Topaz now that its official transaction indexer is live. Posts made on Topaz during the transition window are permanent on-chain and appear once the backend finishes re-indexing.
- **Behind the scenes**, every server-side default that still pointed at Testnet 13 (RPC endpoints, chain ids, indexer URLs, the feed indexer's starting block) was retargeted to Topaz, and the NFT indexer is paused until the NFT contracts are ceremonially deployed there — so no feature silently reads a dead chain.
...
<!-- categories: memba -->
- **Corrected the published moderation policy, which promised things Memba cannot currently do and understated one thing it can.** It stated that an erasure request "scrubs the projection so a rebuild cannot resurface it" and that operators can sweep old tombstones on-chain — neither is built today. It also said flaggers aren't identified, when flagging is an on-chain action whose address our public moderation API relays; the page not showing it is a display choice, not anonymity, and the policy now says so. The erasure section now describes what actually happens: we blocklist the post, which stops every read path that serves our index and survives a full rebuild, **and** we remove the body from the feed contract's current state — which the old text wrongly said was impossible. It also names the two endpoints that relay the chain directly (`/api/render`, `/api/indexer`) and therefore cannot be filtered by a blocklist, rather than claiming a completeness we don't have; states that the body remains in both our index and our raw-event ledger until scrubbing is built; and gives a real contact address instead of pointing vaguely at "the Samourai team." The public transparency log is now reachable from a link in the feed header, where before it could only be found by typing the URL. No moderation behaviour changed — this corrects what we claimed about it.
### Feed — posting from a network the feed isn't indexed on no longer silently loses the post (2026-07-24)
<!-- categories: memba -->
- **Fixed the feed letting you post from a network where the post could never appear.** The feed contract exists on more than one network, but Memba's timeline is built by watching exactly one of them — so if you switched to another network, you'd be shown Testnet 13's timeline while your own posts went to a different chain entirely. The post genuinely succeeded, cost you gas, and is permanently recorded on-chain — it just never showed up anywhere in Memba, with nothing to tell you why. Flagging and deleting were worse: those actions carry a post's number, and the same number refers to a completely different post on another network, so a flag could land on something you never saw. The composer now explains that the feed is only indexed on Testnet 13 and offers a one-click switch instead of inviting a post, the flag and manage controls are hidden off that network, and the underlying write path refuses outright — so no future screen can reintroduce the problem. Reading the feed from any network is unchanged.
...
### Barricade — an experimental true-3D "front line" renderer, for the bake-off (Phase 0 arm B, off by default, 2026-07-17)
- **Added a real 3D renderer spike for the Barricade game — the same front line in genuine perspective (a receding 3D ground the lanes run down, the Order machines advancing toward a defender's parapet, the rebel at the wall) — behind the `VITE_ENABLE_BARRICADE_3D` flag (off) / `?r3d=1` override, with no user-visible change.** It renders the **same** frozen game through react-three-fiber so a phone side-by-side can finish the "is it *dimensionality* or *composition*?" question next to the 2D and 2.5D arms. The `three` stack is lazy-loaded into an isolated async chunk (a CI gate + the service-worker exclusion keep it out of the main bundle — **0 bytes** for the shipped 2D path), and it is WebGL2-gated (older devices simply get 2D). The shipped 2D renderer stays the default and is unchanged; the deterministic sim, its replay log, and the on-chain verifier are untouched.
...
### Reliability — the NFT media cache can no longer exhaust backend memory (2026-07-16)
- **Hardened the image/metadata proxy cache against a memory-exhaustion flood.** The two in-memory caches were bounded only by entry count (256 each) while each entry can hold up to 15 MB — so a flood of distinct large images (or a busy traffic spike) could balloon them to several gigabytes and OOM-kill the backend, which also runs the on-chain indexers. Each cache is now additionally bounded by total bytes (64 MB each, 128 MB combined), evicting least-recently-used entries by size, and an item larger than the whole budget is never cached. Internal resilience change, no user-facing behaviour difference.
...
### Barricade — groundwork for an experimental 3D "front line" renderer (Phase 0 infra, off by default, 2026-07-16)
- **Landed the isolated scaffolding for an opt-in 3D renderer for the Barricade game — dark behind a new `VITE_ENABLE_BARRICADE_3D` flag (off), with the current 2D renderer unchanged and no user-visible change.** Installs the `three` render core and wires the 3D renderer's cost firewall **ahead of** it: a bundle CI gate fails the build if the three stack ever reaches the eager bundle or the service-worker precache, and the precache-exclusion is pre-configured so it will cost **0 bytes** for the main app and for anyone playing in 2D once the renderer lands. Also adds the sim→renderer coordinate bridge and a ground-tap→**integer**-throw mapping (with a property test that guarantees a 3D aim can never silently misfire). The react-three-fiber / drei / postprocessing packages land with the renderer itself (their point of use). The deterministic sim, its replay log, and the on-chain verifier are untouched.
...
### Feed — the post menu closes on Escape / click-away, and inputs are labelled (2026-07-15)
- **Small feed a11y + UX polish.** The per-post manage menu (Edit / Delete) now closes when you press Escape or click anywhere outside it, instead of only when you click the toggle again — via a new reusable `useDismissable` hook. The composer and the inline edit box also carry proper `aria-label`s (a placeholder isn't a label for screen readers). No behaviour change to posting itself. Behind `VITE_ENABLE_FEED`.
...
### Feed — share a post as a branded card image (2026-07-15)
- **Every post gains a "Share card" action** that renders the post as a clean, on-brand PNG (the "Riso Protest" light identity — warm paper, an ink headline, a red spot rule) and hands it to your device's share sheet for X / Telegram, or on desktop downloads the image and copies the link. It's the active companion to the automatic link-preview card: when you *want* to post about something, you get a proper image, not a bare URL. A hidden or deleted post renders a generic "no longer available" card and never paints its body onto the image. No wallet needed. Behind `VITE_ENABLE_FEED`.
### Feed — shared post links now render a rich preview card (2026-07-15)
- **Off-platform link previews.** When a feed permalink (`…/feed/post/<id>`) is shared to X, Telegram, Discord, Slack, WhatsApp, LinkedIn, or scraped by Google/Bing, a Netlify Edge Function now serves those crawlers a proper Open Graph card (title, description, image) built from the post — so a shared link reads as Memba instead of the generic app chrome. Humans are passed straight through to the app, untouched. Moderated content is safe **by construction**: a hidden or deleted post renders a generic "no longer available" card and never leaks its body, a blocklisted post 404s and falls through to the app, and any backend hiccup fails open to the normal page. Bot-only, and the card is not shared-cached, so a takedown is reflected on the next crawl. Takes effect on the next Netlify deploy.
...
- **Fixed optimistic reconciliation.** A just-posted row was matched to the indexed feed purely by author + body, so posting something identical to one of your own already-loaded posts (or two identical posts in a row) made the fresh row instantly match the OLD one and disappear — it looked like the post failed. Reconciliation now also requires the indexed row to be *newer* than the feed's newest post at the moment you posted, so a duplicate of an older post can't swallow the new one, and two identical posts both stand. A 30-second TTL also clears a stuck optimistic row whose indexed version never appears (e.g. if the realm normalized the text), instead of leaving it forever. Applies to both the home timeline and thread replies. Behind `VITE_ENABLE_FEED`.
### Feed — a reply badge on the nav, so you know to come back (2026-07-13)
- **The Feed nav item now shows how many replies-to-you are unread**, from anywhere in the app — a count pill (capped "9+") on the desktop sidebar and a dot on the mobile Feed tab. Previously the "someone replied to you" surface only appeared once you were already on /feed, so the come-back loop never fired for people who'd navigated away. The badge reuses the same reply-notifications poll as the on-page banner (one request app-wide, not two) and clears the instant you mark those replies seen (a cross-tab event keeps the badge and the banner in sync). Shows nothing when you're disconnected or the feed is off. Behind `VITE_ENABLE_FEED`.
...
### MEMBA: BARRICADE — the "certify on-chain" action (dark) (2026-07-13)
- **Turning a verified run into an on-chain score, opt-in.** Behind a new `VITE_ENABLE_BARRICADE_CERTIFY` (off, and NOT safety-gated — no funds move), the daily results poster gains a **Certify on-chain** action, shown only after a run self-verifies. Play stays entirely no-wallet; the wallet code loads lazily only when a player taps Certify. It runs the standard Memba login ceremony to prove the wallet, then submits the re-simulated run (seed, sim version, input log, and the run's own score + state-hash) to the certify endpoint, which re-simulates it server-side and stores only what it can reproduce. A new read-only `arcade.ts` client also reads the leaderboard realm's daily board over qeval (shape-validated, injection-guarded) for the board views that follow. Nothing is visible yet — the flag is off, the endpoint is dark, and the realm isn't deployed.
- **What puts a verified BARRICADE score on-chain.** Building on the verify worker and the submit endpoint: a day-close attester that, once a day is fully closed (past the submit grace window), writes each wallet's best verified run for that day to the leaderboard realm's competitive board — attester-pays, via a dedicated low-privilege gnokey key (never the deploy multisig). It attests one on-chain entry per address (matching the realm's board granularity, and avoiding its non-improving guard), retires a wallet's lesser runs, retries transient broadcast failures on the next cycle, and bounds each cycle's work. It stays completely **dormant** until the operator sets `MEMBA_ARCADE_ATTESTER_ENABLED` and the key — and the key ceremony (generate + fund the key, add it to the realm's attester allowlist) is an owner step. No behavior change ships enabled.
...
### Feed — old posts show a real time instead of "block 671221" (2026-07-13)
- **Posts created before the deterministic-timestamp migration now get a human time.** Migration 019 added a per-post `block_ts` (the block header time, denormalized at ingest so it survives a rebuild-from-raw), but rows written before it kept `block_ts=0` — and the UI rendered those as the raw height ("block 671221") instead of a relative time. The feed indexer now runs a one-time, bounded backfill at startup: for each distinct pre-migration height it reads the block header time from the same `/block` source ingest uses and fills `block_ts`. It's idempotent (already-stamped rows are never touched, unknown heights stay 0 and are retried on restart) and panic-isolated so it can never stall the tailer. Takes visible effect on the next backend deploy. Backend-only.

### Governance — candidature quest verifier repointed to the guarded realm (2026-07-13)
### MEMBA: BARRICADE — the certify submit endpoint (dark) (2026-07-13)
- **Where a BARRICADE run gets checked before it can be certified.** Building on the verify worker: a new `POST /api/arcade/submit` (off by default — 404 until the operator sets `MEMBA_ARCADE_SUBMIT_ENABLED`) authenticates the wallet, re-simulates the submitted input log, and requires the client's claimed score and state-hash to match the re-simulation before storing anything — a client-claimed number is never stored blind. It derives the mode and day from the seed (a daily seed must be for the live window — today or yesterday UTC — so a future seed can't pre-fill a board that isn't open, and a stale one can't farm a closed one), binds each input log to its first submitter (the same replay-theft guard the realm enforces), and caps submissions per IP and per wallet. Verified runs are stored for day-close attestation. No user-facing surface yet — the attester and the certify UI follow.
...
### MEMBA: BARRICADE — the verify worker behind on-chain certify (2026-07-13)
- **The re-simulation that makes a certified score honest.** Groundwork for certifying BARRICADE runs on-chain: a Node "verify worker" that re-runs a submitted input log through the exact same deterministic sim the client played, so the attester only ever writes a re-simulated result — never a client-claimed number. The worker is the frontend sim itself, esbuild-bundled and embedded in the backend (one implementation, one truth — never a second port that could diverge), run in an isolated `node` subprocess with a wall-clock timeout, an output cap, and a concurrency limit. Input is hardened before the process even starts (event-count and payload-size caps, seed charset, sim-version pinning). A committed-bundle freshness gate and a cross-boundary determinism test (the real worker and the frontend sim must reproduce the same pinned results byte-for-byte) keep the two from ever drifting apart. Nothing is wired to a user surface yet — the submit endpoint, attester, and certify UI follow — so there is no visible change.
...
### Marketplace — escrow & agent-registry fund paths repointed to the guarded realms (2026-07-13)
- **The escrow and agent-credit fund flows now hit the `IsUserCall`-guarded realms.** The escrow builders (`CreateContract`/`FundMilestone`) were bound to `escrow_v2` and the agent registry to `agent_registry` (v1) — both read the attached coins without the direct-user-call guard (the phantom-deposit fund-drain class). They now target `escrow_v3` and `agent_registry_v2` (identical APIs; both successors already live on test13, and both old realms are empty so nothing is lost). The **backend** proxies and the pro-credit check (`analyst.go` `checkProCredits`) are repointed in lockstep, so credit deposits and balance reads stay on the same realm. The old paths stay allowlisted defensively.
...
- **The night gets a life of its own.** Behind `VITE_ENABLE_BARRICADE` (still off), the environment pass: each daily seed picks one of five **ambience plates** (curfew-violet, teargas-dawn, blackout, sodium-haze, cold-front) that retint the sky, lanes, and city windows — the mood shifts with the date while every color that *means* something (threat coding, rally teal, scrap gold) stays fixed; **smoke columns drift off the rooftops** (something is always burning in this city); and the barricade finally **shows what you bought** — arming the neighborhood now puts an actual crowd at the wall, paper heads and raised teal fists that fade as the arming winds down, instead of a status strip. Under the hood the renderer also gains the **hand-inked atlas seam**: when the collection-pipeline art lands, each machine's finished riso plate registers into the same draw contract and the shape-grammar silhouette steps aside — no downstream changes. All render-only; replays untouched.
- **The Panopticon walks.** Behind `VITE_ENABLE_BARRICADE` (still off), the siege gets its apex: every fifth overtime round, a walking watchtower ringed with eyes enters the center street and **cycles every denial The Order owns** — a raised pavise that blunts your fire, a douse that kills your lane flames, a jam that freezes your rally and turret, a lock that seals its street — rotating on a telegraphed clock shown as four pips above its crown. Nothing about it is random: reading the rotation and timing your answer *is* the fight. Deterministic (born-tick clock + presence rules, zero new replay-hash fields), and the fairness sweep is untouched — the apex lives past the arc, where death is the design.
- **The deep siege gets its own machines, and the tower learns to fight back.** Behind `VITE_ENABLE_BARRICADE` (still off): three stretch machines join the overtime pools only — the **Carrier**, a riot van that marches while deploying drones into its lane *and* the neighbors; the **Jammer**, a jam-ring mast that freezes your rally meter and holds the turret in its lane silent (the timer pauses rather than burns) until it's destroyed; and the **Mender**, a welding nurse-drone that trails the column and keeps healing the machine ahead of it — ignore it and the front never dies. The daily arc's roster is untouched; these are the deep rounds' problems. And the **Broadcast Tower now fights in three phases**: jamming your rally as always, then deploying menders to keep itself screened as it takes damage, then — at low health — turning its own array on the barricade and shelling you while you race to finish it. All cadence/presence-derived and integer-deterministic, zero new replay-hash fields; the fairness sweep still clears every real daily seed through the harder boss fight.
- **The Overtime Siege — the run no longer ends when you win.** Behind `VITE_ENABLE_BARRICADE` (still off): felling the Broadcast Tower now pays the win bonus and *opens the siege* — endless, seeded rounds that keep coming with the hp/speed caps lifted, so the machines eventually out-scale any defense and every stand ends the same way: the line falls. How DEEP you stood is the new headline — the results poster and the Wordle-style share card lead with your siege round ("THE LINE HELD · SIEGE r7"), and "the line held" now means you got past the boss before falling. Run length becomes skill-determined on the same shared seed, so scores stay comparable; the fairness proof still guarantees an honest-paced player clears the arc on every real daily seed (the margin is measured through the arc, where fairness is owed — the siege is designed death). Deterministic throughout: siege rounds derive from the seed on demand, zero new replay-hash fields, and the verifier's hard tick ceiling (~15 min) bounds every submission.
