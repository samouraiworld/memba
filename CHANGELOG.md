...
- **Mainnet stays on hold.** On gnoland-1 no transaction is built for this contract and every control stays disabled, even for a connected member. Right before signing, the page asks the wallet for its network again and refuses an unknown chain, another chain than the page's, or gnoland-1. Role and key-recovery proposals for this contract version come later.
