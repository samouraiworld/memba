...
- **The gas budget fits the mainnet realm.** It is now 50M, up from 5M. The old budget ran out of gas on the realm's very first entry. Operators can set `MEMBA_ARCADE_GAS_WANTED` and `MEMBA_ARCADE_GAS_FEE_UGNOT`. A fee above `MEMBA_ARCADE_MAX_GAS_FEE_UGNOT` (default 0.2 GNOT) keeps the attester off, with an error in the log.
